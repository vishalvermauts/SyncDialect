...
class TranslationEngine(
    ...
    suspend fun translateTextStreaming(
        sourceLang: String,
        targetLang: String,
        inputText: String,
        onLanguageDetected: (String) -> Unit,
        ...
        withContext(Dispatchers.IO) {
            ...
            try {
                Log.d("TranslationEngine", "Translating text: $inputText")

                val samplerConfig = SamplerConfig(
                    topK = 40, topP = 0.9, temperature = 0.4, seed = 0
                )
                
                val instructionText = "You are a translation assistant. Strictly translate the following text to $targetLang. Output the translation exactly once. Do not repeat phrases. Do not answer questions."

                val convConfig = ConversationConfig(
                    samplerConfig = samplerConfig,
                    systemInstruction = Contents.of(Content.Text(instructionText))
                )
                conversation = engine!!.createConversation(convConfig)

                val cueText = "Text to translate: $inputText\n\n$targetLang translation:"
                val contents = Contents.of(Content.Text(cueText))
                ...
                conversation.sendMessageAsync(contents)
                    .takeWhile { charCount < 1000 } // larger limit for text
                    .collect { chunk: Message ->
                        val text = chunk.contents.contents
                            .filterIsInstance<Content.Text>()
                            .joinToString("") { it.text }
                ...
                Log.d("TranslationEngine", "Text translation complete. Chars: $charCount")
            } catch (e: Exception) {
                Log.e("TranslationEngine", "Inference error: ${e.message}", e)
                withContext(Dispatchers.Main) {
                    onError("Translation failed: ${e.message?.take(100)}")
    ...
    suspend fun translateImageStreaming(
        sourceLang: String,
        targetLang: String,
        ...
        try {
            Log.d("TranslationEngine", "Running ML Kit OCR on Image")
            val recognizer = com.google.mlkit.vision.text.TextRecognition.getClient(
                com.google.mlkit.vision.text.latin.TextRecognizerOptions.DEFAULT_OPTIONS
            )
            val image = com.google.mlkit.vision.common.InputImage.fromBitmap(bitmap, 0)
            
            // Wait for OCR to complete using callbacks to avoid needing play-services coroutine dependency
            val extractedText = kotlinx.coroutines.suspendCancellableCoroutine<String> { continuation ->
            ...
            if (extractedText.isEmpty()) {
                withContext(Dispatchers.Main) { onError("No readable text found in image.") }
                return
            }
            
            Log.d("TranslationEngine", "Extracted text: $extractedText")
            
            // Pipe the extracted text into the standard text translation function
            ...
            Log.e("TranslationEngine", "OCR / Translation error: ${e.message}", e)
            withContext(Dispatchers.Main) {
                onError("Image translation failed: ${e.message?.take(100)}")
            }
        }
    }
